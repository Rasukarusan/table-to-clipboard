...
exports.tsvToHtmlTable = tsvToHtmlTable;
...
function parseSpaceSeparated(data) {
    return data.trim().split("\n").map((line) => {
        // 2つ以上の連続スペースで分割し、各セルをtrim
        return line.split(/  +/).map((cell) => cell.trim());
    });
}
function toHtmlTable(data, hasHeader = true, delimiter = "auto") {
...
// 後方互換性のため
function tsvToHtmlTable(tsv, hasHeader = true) {
...
function copyHtmlToClipboard(html, plainText) {
    ...
    try {
        // HTMLをRTFに変換
        (0, child_process_1.execSync)(`textutil -convert rtf -output "${tempRtf}" "${tempHtml}"`);
...
`;
        ...
        (0, child_process_1.execSync)(`swift "${tempSwift}"`);
    }
    catch (error) {
        ...
        if (fs.existsSync(tempRtf))
            fs.unlinkSync(tempRtf);
        if (fs.existsSync(tempText))
            fs.unlinkSync(tempText);
        if (fs.existsSync(tempSwift))
            fs.unlinkSync(tempSwift);
    }
}
...
//# sourceMappingURL=table-to-clipboard.js.map